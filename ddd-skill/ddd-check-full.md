...
Run when user asks "full DDD audit", "check entire project", "architecture review", or "DDD compliance report".
...
Read EVERY rule file at `ddd-skill/rules/`:
- `domain-layer.md`
...
- `ddd-skill/references/base-classes-reference.md`
...
- [ ] Commands extend `Command<R>`, Queries extend `Query<R>` or `PageQuery<R>`
- [ ] Each use case has dedicated Executor (`AppServiceExecutorI`)
- [ ] Query executors bypass Domain (call Mapper directly)
- [ ] Response always wrapped in `Response` subtype
...
- Always reference the specific rule section when flagging violations (e.g. "domain-layer.md Section A.3")
